...
func TestMain(m *testing.M) {
	...
	// Check if external Redis is provided
	externalRedisAddr := os.Getenv("REDIS_TEST_ADDR")
	externalRedisPassword := os.Getenv("REDIS_TEST_PASSWORD")

	if externalRedisAddr != "" {
		// Use external Redis
		fmt.Printf("Using external Redis at: %s\n", externalRedisAddr)
		redisStore = NewRedisStore(externalRedisAddr, externalRedisPassword)

		// Test connection
		if err := testRedisConnection(redisStore); err != nil {
			fmt.Printf("Failed to connect to external Redis: %v\n", err)
			os.Exit(1)
		}
	} else {
		// Try to use testcontainers
		var err error
		redisContainer, err = NewRedisContainer(ctx, &testing.T{})
		if err != nil {
			fmt.Printf("Failed to create Redis container: %v\n", err)
			fmt.Printf("To run integration tests, either:\n")
			fmt.Printf("1. Install Docker/Podman and ensure it's running\n")
			fmt.Printf("2. Set REDIS_TEST_ADDR and REDIS_TEST_PASSWORD environment variables to use external Redis\n")
			fmt.Printf("Example: REDIS_TEST_ADDR=localhost:6379 REDIS_TEST_PASSWORD=mypass go test -tags=integration\n")
			os.Exit(1)
		}

		// Create Redis store using container
...
// testRedisConnection verifies that Redis is reachable
func testRedisConnection(store *RedisStore) error {
...
func setupTest(t *testing.T) {
	...
	if redisContainer != nil {
		...
		// For external Redis, flush manually
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
		err := redisStore.client.FlushDB(ctx).Err()
		require.NoError(t, err)
	}
}
