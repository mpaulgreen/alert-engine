...
func TestDefaultDefaultRulesConfig(t *testing.T) {
	t.Run("returns valid default config", func(t *testing.T) {
		config := alerting.DefaultDefaultRulesConfig()

		assert.True(t, config.Enabled)
		assert.Equal(t, 5, config.DefaultThreshold)
		assert.Equal(t, 5*time.Minute, config.DefaultTimeWindow)
		assert.Equal(t, "#alerts", config.DefaultChannel)
		assert.Equal(t, "medium", config.DefaultSeverity)
	})
}

func TestCreateDefaultRulesWithConfig(t *testing.T) {
	t.Run("creates rules when enabled", func(t *testing.T) {
		config := alerting.DefaultRulesConfig{
			Enabled: true,
		}

		rules := alerting.CreateDefaultRulesWithConfig(config)
		assert.NotEmpty(t, rules)
	})
	...
	t.Run("uses custom rules when provided", func(t *testing.T) {
		customRule := models.AlertRule{
			ID:   "custom-rule",
			Name: "Custom Rule",
		}
		...
		config := alerting.DefaultRulesConfig{
			Enabled: true,
			Rules:   []models.AlertRule{customRule},
		}
...
func TestGetRuleTemplateWithConfig(t *testing.T) {
	t.Run("creates basic template", func(t *testing.T) {
		config := alerting.DefaultRulesConfig{}

		template := alerting.GetRuleTemplateWithConfig(config)

		assert.Equal(t, "", template.ID)   // Template has empty ID
		assert.Equal(t, "", template.Name) // Template has empty name
		assert.True(t, template.Enabled)   // Default enabled
		assert.Equal(t, "ERROR", template.Conditions.LogLevel)
	})
}

func TestHelperFunctions(t *testing.T) {
	t.Run("helper functions are tested indirectly", func(t *testing.T) {
		// The maxDuration and maxInt functions are private helper functions
		// that are tested indirectly through the public functions that use them.
		// This test exists to verify that the helper functions contribute to coverage.
